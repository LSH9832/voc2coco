...
def get_file_list(image_dirs, annotation_dirs, image_type="jpg"):
    ...
    for anno_dir in annotation_dirs:
        for anno_name in sorted(glob("%s/*.xml" % anno_dir) + glob("%s/*.XML" % anno_dir)):
...
class COCO:
    ...
    def show_each_category_num(self, width=50):
        """
        ...
        neck = "╞%s╤%s╡" % ("═" * (width - 1 - int(width / 3)), "═" * int(width / 3))
...
def convert_only(image_dirs, annotation_dirs, image_type, data_name, coco_image_dir, coco_annotation_dir, class_file, json_file, **_):
    # convert and divide
...
def main():
    ...
    # VOC
    image_dirs = []
    [image_dirs.append(dir_name) if os.path.isdir(dir_name) and dir_name not in [".", ".."] else None
     for dir_name in iglob("*")]
    annotation_dirs = ["%s/annotations" % dir_name for dir_name in image_dirs]
    image_type = "jpg"

    # COCO
    data_name = "GUNS2022"
    coco_image_dir = "/home/bitcq/dataset/%s/images" % data_name
    coco_annotation_dir = "/home/bitcq/dataset/%s/annotations" % data_name
    ...
    ###########################################################################
    """
    do not edit following code unless you know what are you doing
    """
